...
export class DashboardToolHandlers {
  ...
  getToolSchemas(): Tool[] {
    return [
      ...
      },
      {
        ...
        description:
          "Deep-copy a dashboard and all its cards into a target collection. Creates independent copies of every card, preserves the dashboard layout (card positions/sizes), filters, and filter wiring. The copied dashboard has zero references to the original cards.",
        inputSchema: {
          type: "object",
          properties: {
            dashboard_id: {
            ...
            name: {
              type: "string",
              description:
                "Name for the new dashboard. If omitted, uses the original name.",
            },
            collection_id: {
              type: "number",
              description:
                "Target collection ID for the new dashboard and its cards. If omitted, copies to the same collection as the original.",
            },
          },
          required: ["dashboard_id"],
        },
  ...
  private async duplicateDashboard(args: any): Promise<any> {
    ...
    const fullDashboard = await this.client.getDashboard(
      (newDashboard as any).id
    );

    // Build a summary with the card ID mapping
    const dashcards = (fullDashboard.dashcards || []).map((dc: any) => ({
      dashcard_id: dc.id,
      card_id: dc.card_id,
      card_name: dc.card?.name || null,
      row: dc.row,
    ...
    const result = {
      new_dashboard_id: (newDashboard as any).id,
      name: (fullDashboard as any).name,
      collection_id: (fullDashboard as any).collection_id,
      parameters: (fullDashboard as any).parameters || [],
      dashcards,
    };
